...
def minibatch_parse(sentences, model, batch_size):
    ...
    partialParses = [PartialParse(s) for s in sentences]
    unfinishedParses = partialParses[:]

    while len(unfinishedParses) > 0:
        batchParses = unfinishedParses[:batch_size]

        transitions = model.predict(batchParses)

        for (index, parse) in enumerate(batchParses):
            parse.parse_step(transitions[index])
            if len(parse.stack) == 1 and len(parse.buffer) == 0:
                unfinishedParses.remove(parse) # finish parsing

    dependencies = [parse.dependencies for parse in partialParses]
