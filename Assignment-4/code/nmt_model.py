...
class NMT(nn.Module):
    ...
    def encode(self, source_padded: torch.Tensor, source_lengths: List[int]) -> Tuple[torch.Tensor, Tuple[torch.Tensor, torch.Tensor]]:
        ...
        init_decoder_cell = self.h_projection(torch.cat((last_cell[0], last_cell[1]), dim=1))
    ...
    def decode(self, enc_hiddens: torch.Tensor, enc_masks: torch.Tensor,
        ...
        for y_t in torch.split(Y, 1):
            ...
            combined_outputs.append(o_t) # [tensor(b, h)] * tgt_len
            o_prev = o_t # (b, h)
    ...
    def step(self, Ybar_t: torch.Tensor,
        ...
        alpha_t = torch.unsqueeze(F.softmax(e_t, dim=1), dim=1) # (b, 1, src_len)

        # (b, src_len, 1) * (b, src_len, 2h) => (b, 1, 2h) => (b, 2h)
